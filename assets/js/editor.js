function brandEnforcerEditorInitDomElements() {
  ...
  brandEnforcerBuildEditorContainer();
  brandEnforcerBuildEditorHeader();
  brandEnforcerBuildMenu();

  // Attach click events to the menu.
  brandEnforcerMenuClickTree();
  brandEnforcerMenuClickElement()
  ...
  footerColRight.id = 'brand-enforcer-editor-export';
...
function brandEnforcerMenuClickTree() {

  const el = document.getElementById( 'brand-enforcer-editor-menu-tree' );
  ...
  if( el ) {
    ...
    el.addEventListener( 'click', e => {
      ...
      brandEnforcerStylesRemove();
      brandEnforcerInserterRemove();
      brandEnforcerInserterFormRemove();

      // Build tree then add click event handler.
      brandEnforcerTreeAdd();
      brandEnforcerTreeItemClickEvent();
...
function brandEnforcerInserterFormRemove() {

  const el = document.getElementById( 'brand-enforcer-editor-inserter-element-form' );
  if( el ) {
    el.remove();
  }
...
function brandEnforcerMenuClickElement() {

  const el = document.getElementById( 'brand-enforcer-editor-menu-element' );
  ...
  if( el ) {
    ...
    el.addEventListener( 'click', e => {
      ...
      brandEnforcerInserterRemove();
      brandEnforcerSelectElementByDoubleClick();
      brandEnforcerInserterFormRemove();
      brandEnforcerEditElementButtonRemove();
...
function brandEnforcerBuildEditorContainer() {

  // Editor main container.
  let editorContainer = document.createElement( 'div' );
  editorContainer.id = 'brand-enforcer-editor';
...
function brandEnforcerSelectElementByDoubleClick() {

  window.addEventListener( 'dblclick', event => {

    let element = document.getElementById( 'brand-enforcer-editor' );
...
function brandEnforcerTreeItemClickEvent() {

  /* Event listener for clicks on nodes/elements listed in the tree view. */
  const treeElements = document.querySelectorAll( '#brand-enforcer-editor-body li' );
  treeElements.forEach( treeEl => {
    ...
    treeEl.addEventListener( 'click', event => {
      ...
      brandEnforcerElementSelection( targetId, event.target );

      const targetPosition = targetEl.getBoundingClientRect();
      const jsonElementMatch = brandEnforcerFindJsonDefinition( targetId );

      /* Add element edit button. */
      const existingButton = document.getElementById( 'brand-enforcer-edit-element-button' );
      if( existingButton ) {
        existingButton.remove();
      }
      const editElementButtonEl = document.createElement( 'button' );
      editElementButtonEl.id = 'brand-enforcer-edit-element-button';
      editElementButtonEl.innerHTML = 'Edit Element';
      document.getElementById( 'brand-enforcer-editor-body' ).appendChild( editElementButtonEl );
      brandEnforcerEditElementButtonClickEvent( jsonElementMatch );

      /* Add element inserter. */
      brandEnforcerInserterInit();

      /* Add style inputs. */

      /*
      brandEnforcerStylesRemove();
      const styleInputs = brandEnforcerBuildStyleInputs( jsonElementMatch );
      const editorBodyEl = document.getElementById( 'brand-enforcer-editor-body' );
      editorBodyEl.appendChild( styleInputs );
      brandEnforcerStyleInputBlur(); // Attach blur event to process inputs.
      */
      ...
      overlayEl.id = 'brand-enforcer-editor-selection';
      ...
      if( existingOverlay = document.getElementById( 'brand-enforcer-editor-selection' ) ) {
...
function brandEnforcerEditElementButtonClickEvent( jsonElementMatch ) {

  const el = document.getElementById( 'brand-enforcer-edit-element-button' );
...
function brandEnforcerStylesRemove() {

  const existingStylesContainer = document.getElementById( 'brand-enforcer-editor-styles' );
...
function brandEnforcerTreeRemove() {

  console.log( 'removing tree...')

  const editorTree = document.getElementById( 'brand-enforcer-editor-tree' );
  if( editorTree ) {
    editorTree.remove();
  }
 ...
 * Tree Functions.
 */
function brandEnforcerTreeAdd() {

  console.log( brandEnforcerTrees )
  ...
  brandEnforcerTrees.forEach( elementTree => {

    brandEnforcerTreeParseJsonElementsRecursive( elementTree.elements, treeContainer );

  });

  const editorBody = document.getElementById( 'brand-enforcer-editor-body' );
  editorBody.appendChild( treeContainer );

}

function brandEnforcerTreeParseJsonElementsRecursive( elements, treeContainer ) {
  ...
  elements.forEach( element => {
    ...
    if( element.hasOwnProperty( 'elements' ) && element.elements.length > 0 ) {
      brandEnforcerTreeParseJsonElementsRecursive( element.elements, listEl );
    }
...
function brandEnforcerGetSupportedCssProperties() {
 ...
 */
function brandEnforcerStylesConvertElementArrayToObject( styles ) {
...
function brandEnforcerBuildStyleInputs( jsonElement ) {

  const container = document.createElement( 'div' );
  container.id = 'brand-enforcer-editor-styles';

  const cssProps = brandEnforcerGetSupportedCssProperties();

  const elementStylesObject = brandEnforcerStylesConvertElementArrayToObject( jsonElement.styles );
...
function brandEnforcerStyleInputBlur() {

  const styleInputs = document.querySelectorAll('#brand-enforcer-editor-styles input');

  styleInputs.forEach( inputEl => {

    inputEl.addEventListener( 'blur', ( event ) => {

      brandEnforcerSaveStyle( event );
...
function brandEnforcerSaveStyle( event ) {
  ...
  const elementStylesObject = brandEnforcerStylesConvertElementArrayToObject( jsonElementMatch.styles );
  ...
  targetReplacementEl = brandEnforcerRenderer( jsonElementMatch );
  ...
  // targetEl.remove();
  // brandEnforcerRenderer( jsonElementMatch, targetParentEl );

}

// Element ID is needed to search the entire JSON tree.
// We're returning the original object here, not a copy.
function brandEnforcerFindJsonDefinition( targetId ) {

  let jsonElementMatch = false;
  brandEnforcerTrees.every( elementTree => {

    jsonElementMatch = brandEnforcerFindJsonDefinitionRecursive( elementTree.elements, targetId );
    if( jsonElementMatch ) {
      return false;
    }
...
function brandEnforcerFindJsonDefinitionRecursive( elements, targetId ) {
  ...
  elements.forEach( element => {
    ...
    if( ! jsonElementMatch && undefined !== element.elements && element.elements.length > 0 ) {
      jsonElementMatch = brandEnforcerFindJsonDefinitionRecursive( element.elements, targetId );
    }
...
function brandEnforcerElementSelection( elementId, editorItem ) {

  const currentSelectedElements = document.querySelectorAll( '.brand-enforcer-editor-selected' );

  console.log( currentSelectedElements )

  currentSelectedElements.forEach( selectedElement => {
    selectedElement.classList.remove('brand-enforcer-editor-selected');
  });

  // Add selected class to the editor item clicked.
  editorItem.className = 'brand-enforcer-editor-selected';

  console.log( window.brandEnforcerData );

  window.brandEnforcerData.editorSelectedItem = {
    elementId: elementId,
...
function brandEnforcerInserterInit() {

  const existingEl = document.getElementById( 'brand-enforcer-editor-inserter' );
  if( existingEl ) {
    existingEl.remove();
  }

  const inserterContainer = document.createElement( 'div' );
  inserterContainer.id = 'brand-enforcer-editor-inserter';

  const inserterButton = document.createElement( 'div' );
  inserterButton.id = 'brand-enforcer-editor-inserter-button';
  ...
  const editorBodyContainer = document.getElementById( 'brand-enforcer-editor-body' );
  editorBodyContainer.appendChild( inserterContainer );

  brandEnforcerInserterClickEvent();

}

function brandEnforcerInserterShowForm() {

  // Container div.
  const inserterFormContainerEl = document.createElement( 'div' );
  inserterFormContainerEl.id = 'brand-enforcer-editor-inserter-element-form';

  // Render current selected tag.
  if( window.brandEnforcerData.editorSelectedItem.elementId ) {
    const selectedElementId = window.brandEnforcerData.editorSelectedItem.elementId;
    const selectedElement   = document.getElementById( selectedElementId );
    const tagSelectionEl = document.createElement( 'div' );
    tagSelectionEl.id = 'brand-enforcer-editor-inserter-selection-display';
  ...
  tagContainer.id = 'brand-enforcer-inserter-tag-container';
  ...
  tagInputEl.id = 'brand-enforcer-tag-input';
  tagContainer.appendChild( tagInputEl );
  inserterFormContainerEl.appendChild( tagContainer );

  // Position selector.
  const positionEl = document.createElement( 'div' );
  positionEl.innerHTML = '<ul id="brand-enforcer-editor-insert-position-choices"><li data-position="before">BEFORE</li><li data-position="inside">INSIDE</li><li data-position="after">AFTER</li></ul>';
  ...
  statementEl.id = 'brand-enforcer-editor-inserter-statement';
  statementEl.innerHTML = 'create  \u2770div\u2771 before #great-list';
  inserterFormContainerEl.appendChild( statementEl );

  // Add to editor body.
  const containerEl = document.getElementById( 'brand-enforcer-editor-body' );
  containerEl.appendChild( inserterFormContainerEl );

  // Init position choices event.
  brandEnforcerInserterPositionChoicesClickEvent();

}

function brandEnforcerInserterPositionChoicesClickEvent() {

  console.log('calling brandEnforcerInserterPositionChoicesClickEvent()...')

  const els = document.querySelectorAll( '#brand-enforcer-editor-insert-position-choices li' );
  ...
  els.forEach( el => {
    el.addEventListener( 'click', event => {
      ...
      const currentSelected = document.querySelector( '#brand-enforcer-editor-insert-position-choices li.is-selected' );
      if( currentSelected ) {
...
function brandEnforcerEditElementButtonRemove() {
  const elementEditButton = document.getElementById( 'brand-enforcer-edit-element-button' );
 ...
 * Inserter Functions.
 */
function brandEnforcerInserterClickEvent() {

  const inserterEl = document.getElementById( 'brand-enforcer-editor-inserter-button' );
  inserterEl.addEventListener( 'click', event => {
    ...
    brandEnforcerInserterShowForm();

    // Show the confirm button.
    brandEnforcerInserterShowConfirmButton();
...
function brandEnforcerInserterShowConfirmButton() {

  const containerEl = document.getElementById( 'brand-enforcer-editor-inserter-element-form' );
  const inserterConfirm = document.createElement( 'button' );
  inserterConfirm.id = 'brand-enforcer-editor-inserter-confirm-button';
  inserterConfirm.innerHTML = 'Add Element';
  containerEl.appendChild( inserterConfirm );

  /* Attach the click event. */
  brandEnforcerInserterConfirmButtonEvent();

}

function brandEnforcerInserterConfirmButtonEvent() {

  const el = document.getElementById( 'brand-enforcer-editor-inserter-confirm-button' );
  el.addEventListener( 'click', event => {

    // Get the position choice (before, after, inside).
    const positionChoiceEl = document.querySelector( '#brand-enforcer-editor-insert-position-choices li.is-selected' );
    const positionChoice = positionChoiceEl.getAttribute( 'data-position' );

    // Insert the new element into the DOM.
    const insertTag = document.getElementById( 'brand-enforcer-tag-input' ).value;
    if( insertTag === 'text' ) {
    ...
    if( window.brandEnforcerData.editorSelectedItem && window.brandEnforcerData.editorSelectedItem.elementId ) {

      targetElementParent = document.getElementById( window.brandEnforcerData.editorSelectedItem.elementId );
    ...
    brandEnforcerTrees.push( elementTree );

    // Rebuild the tree in the editor so the new element is shown.
    brandEnforcerEditorInit();

    // Set the new element as the current target selection.
    const treeItemMatcherString = '#brand-enforcer-editor-tree li[data-target*="' + newEl.id + '"]';
    window.brandEnforcerData.editorSelectedItem = {
      elementId: newEl.id,
...
function brandEnforcerInserterRemove() {

  const el = document.getElementById( 'brand-enforcer-editor-inserter' );
  if( el ) {
    el.remove();
  }
...
function brandEnforcerElementDeleterClickEvent() {

  const el = document.getElementById( 'brand-enforcer-delete-icon' );
  el.addEventListener( 'click', event => {

    console.log('delete click...')

    const selectedElement = document.getElementById( window.brandEnforcerData.editorSelectedItem.elementId );
...
function brandEnforcerEditorInit() {

  const editor = document.getElementById( 'brand-enforcer-editor' );
  if( editor ) { editor.remove(); }

  brandEnforcerEditorInitDomElements();

  // Build tree then add click event handler.
  brandEnforcerTreeAdd();
  brandEnforcerTreeItemClickEvent();

  brandEnforcerElementDeleterClickEvent();
...
dragObj.init('#brand-enforcer-editor-tree');
